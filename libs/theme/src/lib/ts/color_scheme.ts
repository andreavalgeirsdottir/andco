...
export const getColorScheme = (): Theme['scheme'] | null => {
	return dataAttributeElement?.getAttribute('data-scheme') as Theme['scheme'];
};
...
export const setColorScheme = (theme: Theme['scheme']) => {
	...
	return dataAttributeElement?.setAttribute('data-scheme', theme);
};

/**
 * Add event listener to the document to detect when the user changes the theme
 * Sets a data attribute `data-scheme` on the `<body>` element
 */
export const useColorSchemeFromPrefersColorSchemeEffect = (forceScheme?: Theme['scheme']) => {
	...
	useEffect(() => {
		...
		if (forceScheme) {
			setColorScheme(forceScheme);
			setGlobalColorScheme(forceScheme);
			return;
		}
...
export type Levels = 0 | 1 | 2 | 3 | 4 | 5;
