 ...
 * @see https://chromatone.vercel.app/
 */
...
const { defaultSchema, createMediaPrefers, createTsTypes } = cli.flags;
...
const load = async () => {
	try {
		...
		const schemes = formattedStyles.reduce((_styles, s) => {
			const cleanedKeys = s.split('--c-schemes-')[1];
		...
		if (createMediaPrefers) {
			sassOutputData = schemeEntries.reduce((_styles, [schema, values]) => {
			...
			infoMinor(
				`Generated media queries for schemes: [${schemeEntries
					.flatMap(([key]) => key)
					.join(', ')}]\n and levels: [${Array.from(Array(maxLevel + 1).keys()).join(', ')}]`,
			);
		}
